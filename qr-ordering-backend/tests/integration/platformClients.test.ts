...
describe('platform clients (super-admin)', () => {
  it('rejects a non-super-admin with 403', async () => {
    ...
    expect((await api().get('/api/admin/platform/clients').set(auth(data.token))).status).toBe(403);
  });
  ...
  it('creates a client with a first outlet + a working owner login', async () => {
    ...
    const res = await api().post('/api/admin/platform/clients').set(auth(token)).send({
    ...
    const list = (await api().get('/api/admin/platform/clients').set(auth(token))).body.data;
  ...
  it('edits a client, adds an outlet, edits an outlet, and applies a plan to all', async () => {
    ...
    const upd = await api()
      .patch(`/api/admin/platform/clients/${clientId}`)
      .set(auth(token))
    ...
    const withOutlet = await api()
      .post(`/api/admin/platform/clients/${clientId}/outlets`)
      .set(auth(token))
    ...
    const edited = await api()
      .patch(`/api/admin/platform/outlets/${outletId}`)
      .set(auth(token))
    ...
    const applied = await api()
      .post(`/api/admin/platform/clients/${clientId}/apply-plan`)
      .set(auth(token))
  ...
  it('404s an unknown client and rejects a duplicate owner email', async () => {
    const token = await superAdmin();
    expect((await api().get('/api/admin/platform/clients/nope').set(auth(token))).status).toBe(404);

    const email = `dup_${uid()}@test.local`;
    const first = await api().post('/api/admin/platform/clients').set(auth(token)).send({
      clientName: 'A',
      outletName: 'A1',
      adminEmail: email,
      adminPassword: 'password12345',
    ...
    const second = await api().post('/api/admin/platform/clients').set(auth(token)).send({
      clientName: 'B',
      outletName: 'B1',
      adminEmail: email,
      adminPassword: 'password12345',
  ...
  it('lets a super-admin impersonate an outlet (token scoped to it)', async () => {
    ...
    const tables = await api().get('/api/admin/tables').set(auth(impToken));
  ...
  it('forbids a non-super-admin from impersonating', async () => {
    const { data } = await registerTenant();
    const res = await api()
      .post('/api/admin/platform/outlets/whatever/impersonate')
      .set(auth(data.token));
