...
async function configure(token: string, patch: Record<string, unknown>) {
  const res = await api().patch('/admin/loyalty/config').set(auth(token)).send(patch);
  expect(res.status).toBe(200);
  return res.body.data;
}

async function createMember(token: string, body: Record<string, unknown>) {
  return api().post('/admin/loyalty/members').set(auth(token)).send(body);
}
...
describe('loyalty — program config', () => {
  it('round-trips config incl. the tier ladder', async () => {
    ...
    const got = (await api().get('/admin/loyalty/config').set(auth(token))).body.data;
...
describe('loyalty — members', () => {
  it('enrols a member with a welcome bonus + tier, logged in the ledger', async () => {
    ...
    const detail = (await api().get(`/admin/loyalty/members/${res.body.data.id}`).set(auth(token)))
      .body.data;
  ...
  it('finds a member by a loosely-typed phone search', async () => {
    ...
    const found = (await api().get('/admin/loyalty/members?search=0123456789').set(auth(token)))
      .body.data;
  ...
  it('adjusts points (balance + ledger move together) and refuses to go below zero', async () => {
    ...
    const up = await api()
      .post(`/admin/loyalty/members/${m.id}/adjust`)
      .set(auth(token))
    ...
    // balance == sum(ledger)
    const detail = (await api().get(`/admin/loyalty/members/${m.id}`).set(auth(token))).body.data;
...
describe('loyalty — reward catalog', () => {
  ...
  it('creates fixed-voucher + free-item rewards and validates them', async () => {
    ...
    const list = (await api().get('/admin/loyalty/rewards').set(auth(token))).body.data;
...
describe('loyalty — tenant isolation', () => {
  it('never exposes one store’s members to another', async () => {
    ...
    const cross = await api().get(`/admin/loyalty/members/${m.id}`).set(auth(b));
    expect(cross.status).toBe(404);

    const bList = (await api().get('/admin/loyalty/members').set(auth(b))).body.data;
