...
export function AuditRow({ entry }: { entry: AuditEntry }) {
  return (
    <TableRow className="border-b border-slate-100 last:border-0">
      <TableCell className="whitespace-nowrap px-4 py-3 text-slate-500">{fmt(entry.createdAt)}</TableCell>
      ...
      <TableCell className="whitespace-nowrap px-4 py-3 font-mono text-xs text-slate-400">
        {entry.ip ?? "—"}
      </TableCell>
    </TableRow>
  );
