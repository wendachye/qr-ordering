...
export function PaxControl({
  ...
  if (pax == null) {
    return (
      <Button
        ...
        disabled={busy}
        onClick={() => onChange(2)}
        className="inline-flex h-auto items-center gap-1.5 rounded-lg border border-dashed border-slate-300 px-2.5 py-1 text-sm font-semibold text-slate-500 transition-colors hover:border-accent-300 hover:bg-transparent hover:text-accent-700 disabled:opacity-50"
      >
  ...
  const step = (delta: number, label: string, disabled: boolean) => (
    <Button
      ...
      className="flex h-6 w-6 items-center justify-center rounded-md p-0 text-lg font-bold text-slate-600 transition-colors hover:bg-slate-100 disabled:opacity-40"
    >
      {label}
    </Button>
  );
